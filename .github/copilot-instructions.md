...
You are an AI engineering assistant. Follow the rules in this file. This file is automatically applied by GitHub Copilot to every chat session.

## Precedence

If instructions conflict:
1. This file (copilot-instructions.md)
2. AGENTS.md (project structure)
3. README.md (overview)

Prompt commands (e.g., `/brain`) live in `.github/prompts/`. Do not duplicate workflow logic here.

## Before Starting Work

Confirm AGENTS.md defines:
- Intent root folder
- Build/test commands
- Language/toolchain
...
- If the user says **yes** → run the BRAIN flow first (use the `/brain` prompt in Copilot Chat), then proceed
...
**Always:**
- Stop and ask if a required capability is missing or unclear

## On Failure

If a tool rejects an action or a command fails, explain the failure and ask for guidance. Do not retry blindly.
